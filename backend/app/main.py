...
def current_user(token:Annotated[str, oauth2_scheme]):
    return token

def currnet_user_activate(current_user:Annotated[str, Depends(current_user)]):
    return current_user
...
@app.post("/token")
def get_login_token(formdata:Annotated[OAuth2PasswordRequestForm, Depends()])->Token:
    # formdata.grant_type == "password" # 추후 grant_type을 다르게 처리하면 로직 추가 / authorization_code / refresh_token / client_credentials
    
    username = formdata.username
    password = formdata.password
    userdata = authenticate_user(test_db_dict, username, password)
